		...
		<div style="float: right; width: 70%">
			<form action="/ifrmDayWorker">
				<table border="1">
					<tr>
						<td>근무일자</td>
						<td><input type="text" id="workDate"></td>
					</tr>
					<tr>
						<td>현장/프로젝트</td>
						<td colspan=2>
							<select name="inputFeildOrProject" id="inputFeildOrProject">
									<option>선택하세요.</option>
									<c:forEach var="feildOrProject" items="${feildOrProjectList}">
										<option value="${feildOrProject.name}">${feildOrProject.name}</option>
									</c:forEach>
							</select> <!-- <input type="button" class="modal_btn" value="목록관리"> -->
										<button type="button" class="modal_btn" >목록관리</button>
						</td>
					</tr>
					...
					<tr>
						<td colspan=2 align=center><input type=submit value="저장">
							&emsp; <input type=button value="내용 지우기"></td>
					</tr>
				</table>
			</form>
		</div>
